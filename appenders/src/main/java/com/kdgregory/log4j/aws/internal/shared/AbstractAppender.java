 ...
 * <p>
 * For the most part, appenders have the same behavior: they initialize,
 * transform log messages, and shut down. Most of the code to do that lives
 * here, with a few hooks that are implemented in the appender proper.
 * <p>
 * Note that some behaviors, such as log rotation, are implemented here even if
 * they are not supported by all appenders. The appenders that do not support
 * those behaviors are responsible for disabling them. For example, an appender
 * that does not support log rotation should throw if {@link #setRotationMode}
 * is called.
 * <p>
 * Note that most of the member variables defined by this class are protected.
 * This is primarily to support testing, but also follows my philosophy of
 * related classes: there is no logical difference between direct access
 * properties; in both cases you're defining an API. Of course, this is a public
 * API for an internal class, so any application code that touches these
 * variables should not be surprised if they cease to exist.
 */
public abstract class AbstractAppender<WriterConfigType> extends AppenderSkeleton {
	...
	// note: these must be explicitly assigned in subclass constructor, because
	// they
	...
	// this object is used for synchronization of initialization and writer
	// change

	private Object initializationLock = new Object();

	// this is used to synchronize access to the queue; queue updates are
	// normally
	...
	/**
	 * Returns the maximum batch delay; see {@link #setBatchDelay}. Primarily
	 * used for testing.
	 */
	...
	/**
	 * Sets the number of unsent messages that will trigger message discard. A
	 * high value is useful when network connectivity is intermittent and/or
	 * overall AWS communication is causing throttling. However, a value that is
	 ...
	 * 10MB.
	 * <p>
	 * Note: at present, discard threshold cannot be changed after creating a
	 * log writer. For appenders that rotate logs, a new configuration value
	 * will be recognized at the time of rotation.
	 */
	...
	/**
	 * Sets the action to take when the number of unsent messages exceeds the
	...
	/**
	 * Sets the rule for log stream rotation, for those appenders that support
	 * rotation. See
	 * {@link com.kdgregory.log4j.aws.internal.shared.RotationMode} for values.
	 * <p>
	 * Attempting to set an invalid mode is equivalent to "none", but will emit
	 * a warning to the Log4J internal log.
	 */
	...
	/**
	 * Sets the log sequence number, used by the <code>{sequence}</code>
	...
	/**
	 * Called just before a writer is created, so that the subclass can perform
	 * substitutions on the configuration.
	 */
	protected abstract WriterConfigType generateWriterConfig();

	/**
	 * Called {@link #append} to ensure that we don't have a single message that
	 * violates AWS batching rules.
	 */
	...
	/**
	 * Called by {@link #initialize} and also {@link #rotate}, to switch to a
	 * new writer. Does not close the old writer, if any.
	 */
	...
	private void rotateIfNeeded(long now) {
		// double-checked locking: avoid contention for first check, but make
